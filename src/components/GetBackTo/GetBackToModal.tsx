    ...
    <div
      ...
      <div
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 20 }}>
          <button onClick={() => updateItem(id, { done: !entry.done, doneAt: !entry.done ? Date.now() : undefined })}
            style={{ ...ghostBtn, flex: 1, background: entry.done ? 'var(--t-surf)' : 'var(--t-success)', color: entry.done ? 'var(--t-txt2)' : 'white', border: entry.done ? '1px solid var(--t-brd)' : 'none' }}>
            {entry.done ? '↺ Mark not done' : '✓ Followed up'}
          </button>

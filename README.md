...
- **Followup table** — on every card, under Waiting for: manual rows (title + notes) plus one auto row per ITSM / custom-system ticket on the card. Two marks per row: **blue ✓ progressed** (strikes the row through, resets itself at 00:00) and **green ✓ done** (hides it; "Show completed" reveals). The ticket's own ✓ and its followup row's green ✓ are one state. Every progress/done is a versioned event and lands in Review's Σ day summary ("Followups completed" / "Followups progressed").
...
- **In scope** — every task (◎ pill next to Kind) and every sub-task (◎ on the row) can be taken out of scope: ▶ Play focus mode then never stars/executes that step, and its next-task queue and the walkthrough skip out-of-scope tasks. Default is in scope; nothing else (feed, table, sprint) changes.

...
- **▣ Hub** (sidebar, no digit) — every ITSM and custom-system ticket across all cards in one place (with sync status, ↗/⧉ open, ✓ not-relevant toggle to hide one), today's cards' communication fields, today's open waiting-for rows, and **get-back-to notes**. Custom systems opt in/out via a "show on the Hub" flag. A dashboard tile shows the combined open-ticket count.
- **Get back to `<who>`** — the whole feature is a name + a note: create one from Spotlight (`⌘F`), Explore's search, `⌘K`, or straight from the Hub; it never enters the card feed, table, or Kanban — it only lives in search and the Hub, with a ✓ to mark it followed up. Logged and versioned exactly like every other edit, and counted in the Review's Σ day summary ("Followed up with" / "Added to get-back-to").

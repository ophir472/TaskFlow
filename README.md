...
- **Followup table** — on every card, under Waiting for: manual rows (title + notes) plus one auto row per ITSM / custom-system ticket on the card. Two marks per row: **blue ✓ progressed** (strikes the row through, resets itself at 00:00) and **green ✓✓✓ done** (hides it; "Show completed" reveals). The ticket's own ✓✓✓ and its followup row's are one state. Every progress/done is a versioned event and lands in Review's Σ day summary ("Followups completed" / "Followups progressed").
- **Get back to `<who>`** — a people-bound note (name + notes), separate from the card-bound followups: create one from Spotlight (`⌘F`), Explore's search, `⌘K`, or straight from the Hub; it never enters the card feed, table, or Kanban — only search and the Hub. The popup's ✓ (top-right) marks it followed up, Done just closes. Counted in the Σ day summary ("Followed up with" / "Added to get-back-to").
- **▣ Hub** (sidebar, no digit) — every ITSM and custom-system ticket across all cards (sync status, ↗/⧉ open, ✓ done), **today's cards' followups** (progressed/done rows disappear until "Show done followups"), **get-back-to notes**, **all followups** collapsed, today's cards' communication fields, and today's open waiting-for rows. Custom systems opt in/out via a "show on the Hub" flag. A dashboard tile shows the combined open-ticket count.
- **In scope** — every sub-task has a ◎ on its row; switch it off and ▶ Play focus mode never stars/executes that step (a task whose open steps are all out of scope is skipped by Play's next-task hop). Default is in scope; nothing else (feed, table, sprint) changes.
...
**Working the queue**
- **Card feed** — one card at a time from a scored queue (urgent / important / quick tags — a collapsed dropdown on existing tasks, chips on the create form; staleness, hold-return boost); a thin frosted transport bar (← back · ⏸ hold · ▶ play · 🎉 complete · continue →); "for today" focus mode
...
- **Sprint war mode** (`s`) — dark one-item-at-a-time blitz through everything quick: pending mail first, then quick tasks & sub-tasks oldest-first; timer, arrow navigation, click-title context, and a ⊞ drawer that opens any relevant field for editing. Settings → Sprint queue: type toggles (mail / quick tasks / quick sub-tasks), drag-to-reorder within sections, click-through to any item
...
- **Review flagging** — a task is queued for review when it was created or edited after its personal `reviewedAt`. Finishing a card's walkthrough stamps it (without counting as an edit); any later change re-flags it. Sessions persist, resume mid-step, and compact walked cards on reopen. The Settings queue supports drag-to-reorder (the walkthrough follows it) and per-card × dismiss (quiet reviewed stamp — the next change re-flags it).
- **Sprint pool** — pending mail first, then quick tasks and quick sub-tasks together oldest-first (each by its own creation time); gated by the Settings type toggles and reordered by the manual drag order (new items join at the end of their section); frozen at start; items completed elsewhere are skipped automatically.

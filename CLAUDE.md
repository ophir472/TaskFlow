...
- `Home` — dashboard landing view (version registry, stat tiles, agenda pipeline, walkthrough start); `WalkthroughBar` — floating store-driven guide (`walkthrough` quiet state); `QuickHelp` — walker over type-'quick' tasks; `Hub` — every ITSM/custom-system ticket + today's followups (+ all followups collapsed) + get-back-to notes + today's communications/waits, in one page; `GetBackToModal` — the one Who/Notes edit surface (✓ top-right completes, Done closes), opened from Explore/Spotlight results and Hub rows
...
- **In scope (2026-08-27):** `Task.inScope?` / `Subtask.inScope?` — `false` opts out, missing = in (no migration). ONLY ▶ Play honors it: `starred`/next-step/`skipNext`/`noSteps`/`allDone` consider in-scope steps; Play's next-task queue and the walkthrough's first-today-card skip out-of-scope tasks. Shared `ScopeToggle` (task pill) + `SubScopeToggle` (row ◎) in `Common/ScopeToggle.tsx`; edits go through `updateItem`/`updateSubtask` (already versioned).
